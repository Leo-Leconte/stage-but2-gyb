...
const ListeStages = () => {
    ...
    useEffect(() => { // dans un useEffect pour que ça fasse le fetch à chaque fois que la page se charge
        ...
        async function fetching(){
            ...
            const reponseApiStagiaire = await fetch(`http://127.0.0.1:3000/api/stagiaire`,{
                method: "GET",
                headers:{
                    "Accept": "application/json",
                    "Content-Type": "application/json",
        ...
        <>
            ...
            <div className={styles.grille}>
                {stages.map((stage: any) => {
                    ...
                    const stagiaire = stagiaires?.find((s: any) => s.id === stage.id_stagiaire);
                    ...
                    return (
                        <div key={stage.id} className={styles.card}>
                            ...
                            <div className={styles.cardBody}>
                                ...
                                <p><strong>Stagiaire :</strong> {stagiaire ? `${stagiaire.prenom} ${stagiaire.nom}` : `ID ${stage.id_stagiaire} introuvable`}</p>
                                <p><strong>Tuteur :</strong> {tuteur ? `${tuteur.prenom} ${tuteur.nom}` : `ID ${stage.id_tuteur} introuvable`}</p>
